...
aij <- matrix(c(-60,60,rep(-2.5,13)),ncol=1) # overall sparsity
bij[,1] <- matrix(c(2,-2,rep(0,13)),ncol=1) # across-group sparsity
...
for(i in 1:numfiles){
	...
	adj[which(upper.tri(adj))] <- c(gij[1:2,i],gij[3:nedge,1])
...
gij[3:nedge,2:numfiles] = gij[3:nedge,1]
...
#		Xk <- t(U%*%sqrt(D)%*%t(Zk))
	...
	setwd(paste("F:\\Paper\\Gen_data\\v4x_seed123\\gendata_p",p,"_v42",sep=""))
