...
aij <- matrix(c(rep(-60,7),rep(60,8)),ncol=1) # overall sparsity
bij[,1] <- matrix(c(rep(1,7),rep(-3,8)),ncol=1) # across-group sparsity
...
#		Xk <- t(U%*%sqrt(D)%*%t(Zk))
	...
	setwd(paste("F:\\Paper\\Gen_data\\v4x_seed123\\gendata_p",p,"_v50",sep=""))
