...
aij <- matrix(c(rep(-60,6),rep(60,3),rep(-5,6)),ncol=1) # overall sparsity
bij[,1] <- matrix(c(rep(2,6),rep(-3,3),rep(0,6)),ncol=1) # across-group sparsity
...
for(i in 1:numfiles){
	...
	adj[which(upper.tri(adj))] <- c(gij[1:9,i],gij[10:nedge,1])
...
gij[10:nedge,2:numfiles] = gij[10:nedge,1]
...
#		Xk <- t(U%*%sqrt(D)%*%t(Zk))
	...
	setwd(paste("F:\\Paper\\Gen_data\\v4x_seed123\\gendata_p",p,"_v49",sep=""))
