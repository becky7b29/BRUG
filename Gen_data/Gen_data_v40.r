...
aij <- rep(-2,15) # overall sparsity
bij[,1] <- rep(0,15) # across-group sparsity
...
for(i in 2:numfiles){
	...
	assign(paste("adj",i,sep=""),adj1)
	
}


gij[,2:numfiles] = gij[,1]
...
#		Xk <- t(U%*%sqrt(D)%*%t(Zk))
	...
	setwd(paste("F:\\Paper\\Gen_data\\v4x_seed123\\gendata_p",p,"_v40",sep=""))
