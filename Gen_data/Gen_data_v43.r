...
aij <- matrix(c(-60,60,60,rep(-2.8,12)),ncol=1) # overall sparsity
bij[,1] <- matrix(c(2,-2,-2,rep(0,12)),ncol=1) # across-group sparsity
...
for(i in 1:numfiles){
	...
	adj[which(upper.tri(adj))] <- c(gij[1:3,i],gij[4:nedge,1])
...
gij[4:nedge,2:numfiles] = gij[4:nedge,1]
...
#		Xk <- t(U%*%sqrt(D)%*%t(Zk))
	...
	setwd(paste("F:\\Paper\\Gen_data\\v4x_seed123\\gendata_p",p,"_v43",sep=""))
